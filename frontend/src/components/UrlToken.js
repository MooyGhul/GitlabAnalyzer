...
function UrlToken() {
  ...
  const createConfigID = async (url, token) => {
      const configUrl = process.env.NODE_ENV === 'development' ? "http://localhost:8080/api/config/" : "/api/config/"
    return axios({
      method: "post",
      url: configUrl,
      data: { url, token },
  ...
  const loadAllProjects = async (configID) => {
      const loadUrl = process.env.NODE_ENV === 'development' ? `http://localhost:8080/api/config/${configID}/load` : `/api/config/${configID}/load`
    return axios({
      method: "post",
      url: loadUrl,
    })
